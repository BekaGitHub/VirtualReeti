...
public class XMLCommandParserSwing extends XMLCommandParser
{
    ...
    public XMLCommandParserSwing(AgentsOnStage stage)
    {
        super(stage);
    }
...
// TODO cut the crap with the two animation types ...
        ...
        if (stickmanname != null)
        {
            a = (a instanceof EventAnimationSwing)
                    ? AnimationLoaderSwing.getInstance().loadEventAnimation(onStage.getAgent(stickmanname), animationname, duration, blocking)
                    : AnimationLoaderSwing.getInstance().loadAnimation(onStage.getAgent(stickmanname), animationname, duration, blocking);
